...
def train_rbf_total(train_file, test_file, standarize, classification, ratio_rbf, l2, eta, fairness, outputs, logisticcv, model, pred):
    ...
    if not pred:    
        ...
        for s in range(1,6,1):
            ...
            train_results, test_results, coeff_number = \
            ...
            if classification:
                ...
                coeffss[s-1] = coeff_number
                print("Number of coeffs: %i" % coeffss[s-1])
        ...
        print("Number of coeffs: ", coeffss)
        print("Average of coefficients: %.2f" % np.mean(coeffss))
        if fairness: 
...
def train_rbf(train_file, test_file, standarize, classification, ratio_rbf, l2, eta, fairness, outputs, logisticcv, model_file=""):
    ...
    if not classification:
        ...
        logreg, coeff_number = logreg_classification(r_matrix, train_outputs, l2, logisticcv, eta)
    ...
    return train_results, test_results, coeff_number 
...
def logreg_classification(matriz_r, train_outputs, l2, logisticcv, eta):
    ...
    if logisticcv:
        logreg = LogisticRegressionCV(Cs=2,  # We can adjust the values or use a list of values for Cs
                                        penalty='l2' if l2 else 'l1',
                                        solver='saga',
                                        verbose=False,
                                        fit_intercept=False,  # You can adjust this based on your requirements
                                        max_iter=10
                                    )
        logreg.fit(matriz_r, train_outputs[:, 0])
    ...
    # Assuming coefficients is a 2D NumPy array
    coefficients = logreg.coef_

    # Set coefficients with absolute values lower than 10^-5 to null
    threshold = 1e-5
    coefficients[np.abs(coefficients) < threshold] = 0

    # Count how many coefficients are greater than 0
    nonzero_coefficients = coefficients[coefficients > 0]
    # Count how many coefficients are greater than 0
    nonzero_count = len(nonzero_coefficients)
    
    return logreg, nonzero_count
