...
def train_rbf_total(train_file, test_file, standarize, classification, ratio_rbf, l2, eta, fairness, outputs, logisticcv, model, pred):
    ...
    if not pred:    
        ...
        train_mean, train_std = np.mean(train_ccrs), np.std(train_ccrs)
        test_mean, test_std = np.mean(test_ccrs), np.std(test_ccrs)

        # Round the values to the closest integer
        train_mean_rounded, train_std_rounded = round(train_mean), round(train_std)
        test_mean_rounded, test_std_rounded = round(test_mean), round(test_std)

        # Print the results
        print("Training CCR: %d%% +- %d%%" % (train_mean_rounded, train_std_rounded))
        print("Test CCR: %d%% +- %d%%" % (test_mean_rounded, test_std_rounded))
        if fairness: 
