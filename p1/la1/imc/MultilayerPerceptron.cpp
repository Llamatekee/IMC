...
void MultilayerPerceptron::backpropagateError(double* target) {
	...
	for(int i=0; i<this->layers[this->nOfLayers-1].nOfNeurons; i++){ // For every neuron
		output = this->layers[nOfLayers-1].neurons[i].out; // Get the output
		this->layers[this->nOfLayers-1].neurons[i].delta = -(target[i]-output)*output*(1-output); // Calculate the delta
	}

	for(int i=this->nOfLayers-2; i>=1; i--){ // For every layer
		for( int j=0; j<this->layers[i].nOfNeurons; j++){ // For every neuron
			output = this->layers[i].neurons[j].out; // Get the output
			aux = 0.0; // Reset aux
			...
			this->layers[i].neurons[j].delta = aux * output * (1 - output); // Calculate the delta
		}
	}
}
